...
namespace cgCourse
{
	void Texture::loadFromFile(const std::string& _filename)
	{
		...
		size = glm::vec2(FreeImage_GetWidth(rawimage), FreeImage_GetHeight(rawimage));
		...
		switch(bitsPerPixel)
		{
			case 24:
			{
				...
				if(!image) std::cout << "Freeimage loading: go mad here" << std::endl;
		...
		/* DONE: generate the textures here:
		...
		// this function releases the memory previously occupied by freeimage for image loading
