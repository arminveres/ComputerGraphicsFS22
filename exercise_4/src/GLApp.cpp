...
struct EventDispatcher {
  ...
  static void mouseButtonCallback(GLFWwindow *_window, int _button, int _action,
                                  int _mods) {
    GLApp *app = static_cast<GLApp *>(glfwGetWindowUserPointer(_window));
    if (app) {
  ...
  static void cursorPosCallback(GLFWwindow *_window, double _xpos,
                                double _ypos) {
    GLApp *app = static_cast<GLApp *>(glfwGetWindowUserPointer(_window));
    if (app) {
...
GLApp::GLApp(glm::uvec2 _windowSize, std::string _title, bool _resizeable)
    : title(_title) {
  // GLFW initialization
  if (!glfwInit()) {
  ...
  window_ = glfwCreateWindow(int(_windowSize.x), int(_windowSize.y),
...
void GLApp::stop() { glfwSetWindowShouldClose(window_, GL_TRUE); }
