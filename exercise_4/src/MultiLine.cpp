...
namespace cgCourse
{
                         ...
                         const std::vector< glm::vec3 >& _normals) : Shape()
	{
		...
		{
            positions.push_back(_vertices[j]);
            positions.push_back(_vertices[j] + (_normals[j] * 0.25f));
    ...
    void MultiLine::initIndexBuffer() {
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, lineIndices.size() * sizeof(glm::uvec2), lineIndices.data(), GL_STATIC_DRAW);
    }
