...
bool ShaderProgram::loadAndCompileShaderToGPU(GLuint _program,
...
bool ShaderProgram::checkLinkStatusOfProgram(GLuint _program) {
  ...
  if (!status) {
    ...
    std::cerr << "Linking error:" << std::endl << errorMessage << std::endl;
    return false;
  }
  return true;
}
...
void ShaderProgram::bind() const { glUseProgram(program); }
