...
bool Cube::createVertexArray(GLuint posAttribLoc, GLuint colAttribLoc,
                             GLuint normAttribLoc) {
  // Initialize buffer objects with geometry data
  glGenBuffers(1, &posBO_);
  glBindBuffer(GL_ARRAY_BUFFER, posBO_);
  glBufferData(GL_ARRAY_BUFFER, sizeof(CubeData::Vertices), CubeData::Vertices,
               GL_STATIC_DRAW);

  glGenBuffers(1, &colBO_);
  glBindBuffer(GL_ARRAY_BUFFER, colBO_);
  glBufferData(GL_ARRAY_BUFFER, sizeof(CubeData::Colors), CubeData::Colors,
               GL_STATIC_DRAW);

  glGenBuffers(1, &normBO_);
  glBindBuffer(GL_ARRAY_BUFFER, normBO_);
  glBufferData(GL_ARRAY_BUFFER, sizeof(CubeData::Normals), CubeData::Normals,
               GL_STATIC_DRAW);
...
void Cube::draw() {
  glBindVertexArray(modelVAO_);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, idxBO_);
  glDrawElements(GL_TRIANGLES, sizeof(CubeData::Triangles) / sizeof(GLuint),
                 GL_UNSIGNED_INT, 0);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
  glBindVertexArray(0);
}
