...
public:
  Circle(float _radius, int _segments) {
    ...
    for (int j = 0; j < segments; j++) {
      ...
      vertices.push_back(
          glm::vec2(radius * std::sin(radians), radius * std::cos(radians)));
    }
  }
  ...
  static float toRadians(float _angle) { return _angle * M_PI / 180.0; }
...
Torus::Torus() : Shape() {
  ...
  int segmentsInXZPlane =
      20; // subdivision in xz-plane (amount of ring segments)
  ...
  for (int j = 0; j < circleXZ.getSegments(); j++) {
    for (int i = 0; i < circleXY.getVertices().size(); i++) {
      glm::vec3 currentPoint(circleXZ.getVertices()[j].x, 0.0,
                             circleXZ.getVertices()[j].y);
      glm::vec3 dirExtension =
          glm::normalize(currentPoint) * circleXY.getVertices()[i].x;
  ...
  for (int j = 0; j < circleXZ.getSegments(); j++) {
    ...
    if (j + 1 == circleXZ.getSegments()) {
      ...
      for (int i = 0; i < circleXY.getSegments(); i++) {
        ...
        if (i + 1 == circleXY.getSegments()) {
          addTwoFaces(currIdx, currIdx + circleXY.getSegments(), startIdx,
                      startIdx + circleXY.getSegments());
        } else {
          addTwoFaces(currIdx, currIdx + circleXY.getSegments(), currIdx + 1,
                      currIdx + 1 + circleXY.getSegments());
        }
      }
    }
  }
  ...
  for (int i = 0; i < faces.size(); i++) {
    glm::vec3 v1 = positions[faces[i].y] - positions[faces[i].x];
    glm::vec3 v2 = positions[faces[i].z] - positions[faces[i].x];
  ...
  for (int i = 0; i < positions.size(); i++) {
    ...
    for (int j = 0; j < faces.size(); j++) {
      if (i == faces[j].x || i == faces[j].y || i == faces[j].z) {
        faceids.push_back(j);
      }
    }
    // sum up the face normals and divide it by the lenght of the summed up
    // vector
    glm::vec3 vSum(0.0, 0.0, 0.0);
    for (int j = 0; j < faceids.size(); j++) {
      vSum += faceNormals[faceids[j]];
    }
    vSum /= glm::length(vSum);
    normals.push_back(vSum);
  }
}

void Torus::addTwoFaces(unsigned int _idx0, unsigned int _idx1,
                        unsigned int _idx2, unsigned int _idx3) {
