...
void Shape::setPosition(glm::vec3 _pos) {
  //...
  translationMatrix = glm::translate(identityMatrix, _pos);
  modelMatrix *= translationMatrix;
}

void Shape::setRotation(float _angle, glm::vec3 _rot) {
  rotationMatrix = glm::rotate(identityMatrix, glm::radians(_angle), _rot);
  modelMatrix *= rotationMatrix;
}

void Shape::setScaling(glm::vec3 _scale) {
  scalingMatrix = glm::scale(identityMatrix, _scale);
  modelMatrix *= scalingMatrix;
}

void Shape::calculateModelMatrix() {
  // done directly in the set functions
}
// TODO End
